...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        features = self.visual_projection(features)[:,None,:]  # (N,1,W)
        # 2
        tgt_mask = torch.tril(torch.ones((T, T)), diagonal=0)
        # 3
        transformer_features = self.transformer(captions, features, tgt_mask)  # (N,T,W)
        scores = self.output(transformer_features)  # (N,T,V)
