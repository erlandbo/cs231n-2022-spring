...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(prev_h @ Wh + x @ Wx + b) # NxH @ HxH + NxD @ DxH + H -> NxH + NxH + NxH -> NxH
...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = dnext_h * (1-next_h**2) @ Wx.T  # (NxH o NxH) @ HxD  -> NxH @ HxD -> NxD
    dprev_h = (dnext_h * (1-next_h**2)) @ Wh.T  # (NxH o NxH) @ HxH.T -> NxH @ HxH -> NxH
    dWx = x.T @ (dnext_h * (1-next_h**2))  # DxN @ (NxH o NxH) -> DxN o NxH -> DxH
    dWh = prev_h.T @  (dnext_h * (1-next_h**2))  # (NxH).T @ (NxH o NxH) -> HxN @ NxH -> HxH
    db = np.sum((dnext_h * (1-next_h**2)), axis=0)  # sum(NxH, dim=0) -> H
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, h, Wx, Wh, b, h0, prev_h  = cache
    N, T, D = x.shape
    H = b.shape
    dx, dh0, db, dWx, dWh, dprev_h = np.zeros_like(x), np.zeros_like(h0), np.zeros_like(b), np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(prev_h)
    for t in range(T-1, -1, -1):      
        if t == 0: cache_t = (x[:,t,:], h0, Wx, Wh, b, h[:,t,:])
        else: cache_t = (x[:,t,:], h[:,t-1,:], Wx, Wh, b, h[:,t,:])
