...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i == k: continue
            else: sum1 += torch.exp(sim(z_k, out[i]) / tau)
        for i in range(2 * N):
            if i == k+N: continue
            else: sum2 += torch.exp(sim(z_k_N, out[i]) / tau)
        l_k = - torch.log(torch.exp(sim(z_k, z_k_N) / tau) / sum1)
        l_k_N = - torch.log(torch.exp(sim(z_k_N, z_k) / tau) / sum2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = out_left.shape
    dot_prod = torch.bmm(out_left.view(N, 1, D), out_right.view(N, D, 1)).squeeze(-1)  # (N,1,D)@(N,D,1)->(N,1,1)->(N,1)
    pos_pairs = dot_prod / (torch.linalg.norm(out_left,dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    # ((2N,D) @ (D,2N)) / ((2N,1) @ (1,2N)) -> (2N,2N) / (2N, 2N) -> (2N,2N)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.exp(sim_matrix[torch.arange(N), torch.arange(N, 2*N)] / tau).repeat(2)[:, None]  # (2N,1)
